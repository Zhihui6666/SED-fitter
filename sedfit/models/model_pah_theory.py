...
tb = np.genfromtxt(template_path+"PAH_template_theory.dat")
wave = tb[:, 0]
fltr1 = (wave < 1000) & (wave > 1)
twave = wave[fltr1]
tflux_temp = tb[:, 1][fltr1]
norm = np.trapz(tflux_temp, twave)
tflux = tflux_temp / norm
tPAH = interp1d(twave, tflux)
waveLim = [np.min(twave), np.max(twave)]

def pah_theory(logLpah, wave, DL, z, frame="rest", t=tPAH, waveLim=waveLim):
    """
...
if __name__ == "__main__":
    ...
    flux = pah_theory(40, wave, DL=30, z=0)
    plt.plot(wave, flux)
    plt.xscale("log")
    plt.yscale("log")
    plt.show()
