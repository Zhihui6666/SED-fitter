...
class ModelFunction(object):
    def __init__(self, function, xName, operation, parFitDict={}, parAddDict={}):
...
class ModelCombiner(object):
    ...
    def combineResult(self, x=None):
        ...
        if x is None:
            x = self.__x
        y = np.zeros_like(x)
        multiY = np.ones_like(x)
        keyList = self.__modelDict.keys()
        for modelName in keyList:
            mf = self.__modelDict[modelName]
            if mf.operation == "multiply":
                multiY *= mf(x)
        for modelName in keyList:
            mf = self.__modelDict[modelName]
            if mf.operation == "plus":
                y += mf(x)
            elif mf.operation == "plus&multiply":
                y += mf(x)*multiY
        return y
    ...
    def componentResult(self, x=None):
        ...
        if x is None:
            x = self.__x
        arrayMultiply = self.__modelDict["Extinction"](x)
        for modelName in self.__modelDict.keys():
            mf = self.__modelDict[modelName]
            if self.__modelDict[modelName].operation == "plus":
                result[modelName] = mf(x)
            elif self.__modelDict[modelName].operation == "plus&multiply":
                result[modelName] = mf(x)*arrayMultiply
    ...
    def plot(self, x=None, colorList=None, FigAx=None, DisplayPars=False,
        ...
        if colorList is None:
            # colorList = ["g", "r", "m", "b", "y", "c"]
            colorList = ["g", "y", "b", "c", "r", "m"]
        nColor = len(colorList)
        counter = 0
        for modelName in modelList:
            if modelDict[modelName].operation != "multiply":
                textList.append("<{0}>\n".format(modelName))
        ...
        if useLabel:
            ...
            tLabel = None
        if tKwargs is None:
            ax.plot(x, yTotal, color="black", label=tLabel)
        else:
...
def Model_Generator(input_model_dict, func_lib, x_list, par_add_dict_all={}, **kwargs):
    ...
    for modelName in modelNameList:
        ...
        modelDict[modelName] = ModelFunction(funcName, xName, operation, parFitDict, parAddDict)
